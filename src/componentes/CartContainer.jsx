...
const CartContainer = () => {
    const { cartList, emptyCart, totalPrice, deleteItem, thereProduct } = useCartContext()
    return (
                ...
                <div className='container item-cart-container'>
                {
                    cartList.map(prodCart => (
                        <div key={prodCart.id} className='d-flex align-items-center justify-content-center p-2 item-container'>
                            ...
                            <div>
                                <CloseSquareFilled onClick={()=>deleteItem(prodCart.id)} style={{ fontSize: '20px', marginLeft: '1rem' }}  />
                            </div>
                        </div>
                    ))
                }       
